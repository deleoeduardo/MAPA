var selected_shape  = null
      ...
      function initialize()
      {
            ...
            drawman         = new goo.drawing.DrawingManager({map:map_in, markerOptions: {icon:'imgRig.png'}}),
				...
				for(var i = 0; i < shapes.length; i++){
					...
					switch(tmp.type){
						case 'CIRCLE':
						   ...
						   tmp.geometry=IO.p_(shape.getCenter());
						   break;
						case 'RECTANGLE':
						   tmp.geometry=IO.b_(shape.getBounds());
						   break;
						case 'POLYLINE':
						   tmp.geometry=IO.l_(shape.getPath(),false);
						   break;
						case 'POLYGON':
						   var resultColor = google.maps.geometry.poly.containsLocation(e.position, shape) ?'red' :'green';
						   alert(resultColor);
						   tmp.geometry=IO.m_(shape.getPaths(),false);
						}
            }
          });
         ...
         function attachPolygonInfoWindow(polygon,tag) {
              ...
              google.maps.event.addListener(polygon, 'mouseover', function (e) {
                  content='<form class="PRUEBA" action="map.html" method="post"><p>  '+
          ...
          goo.event.addListener(drawman, 'polygoncomplete', function(e) {
            e.setOptions({fillColor:'green'});
          });
	      ...
	      /*goo.event.addDomListener(byId('custom'), 'click', function(){
            selected_shape.setOptions({fillColor:byId('custom').value});
          });

          $("#custom").on('change.spectrum', function(e, tinycolor) {
            selected_shape.setOptions({fillColor:tinycolor.toHex()});
          });*/
            ...
            for(var i = 0; i < arr.length; i++)
            {
              ...
              switch(shape.type){
                 case 'CIRCLE':
                  ...
                  break;
                 case 'MARKER':
                    tmp=new goo.Marker({position:this.pp_.apply(this,shape.geometry),icon:'imgRig.png'});
                  break;
                 case 'RECTANGLE':
